...
// ── BASE URL (static env var — safe at module scope) ─────────────────────────
...
// Helper — joins BASE + path without double slashes
...
// ─── 3D Avatar ───────────────────────────────────────────────────────────────
// Receives avatarSrc (already fully resolved URL) — no URL logic inside here.
...
export default function Hero() {
  ...
  // ── State ────────────────────────────────────────────────────────────────
  const [resumePath, setResumePath] = useState('');   // raw path from API
  const [avatarPath, setAvatarPath] = useState('');   // raw path from API
  const [socials, setSocials]       = useState({ github: '', linkedin: '', twitter: '', email: '' });

  // ── Derived URLs (re-computed reactively whenever state changes) ─────────
  // These live INSIDE the component so they always reflect current state.
      ...
      <div style={{
        ...
        >
          ...
          >
            ...
            {/* Resume download */}
            <motion.a
              whileHover={{ scale: resumeHref ? 1.06 : 1 }}
        ...
        <Avatar3D
          theme={theme}
          avatarSrc={avatarSrc}   // ✅ fully resolved URL passed as prop
          avatarRef={avatarRef}
          x={x} y={y}
