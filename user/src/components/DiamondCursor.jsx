...
const SPARK_COLORS = [
  '#ffffff', '#c4b5fd', '#93c5fd', '#f9a8d4',
  '#6ee7b7', '#fcd34d', '#fb923c',
];

function hsl(h, s = 85, l = 65) {
  return `hsl(${h},${s}%,${l}%)`;
}

// ── draw helpers (pure functions, no React deps) ──────────────────────────────

function drawDiamond(ctx, x, y, sz, col, al, rot = 0, glowScale = 3.5) {
...
function drawSparkle(ctx, x, y, sz, al, col) {
  ctx.save();
  ctx.translate(x, y);
  ctx.globalAlpha = al;
  ctx.strokeStyle = col;
...
function drawAura(ctx, x, y, hue, frame) {
  for (let r = 0; r < 3; r++) {
...
function drawConstellation(ctx, trail, hue) {
...
function drawBurst(ctx, b) {
  ...
  for (const l of layers) {
    const r = b.r * l.s;
    for (let j = 0; j < 12; j++) {
      const ang = (j / 12) * Math.PI * 2 + b.r * 0.025;
      drawDiamond(
        ctx,
        b.x + Math.cos(ang) * r,
        b.y + Math.sin(ang) * r,
...
function spawnSparks(sparks, x, y, count) {
...
export default function DiamondCursor() {
  ...
  useEffect(() => {
    ...
    function onClick(e) {
      ...
      spawnSparks(s.sparks, e.clientX, e.clientY, 26);
    }
    ...
    function animate() {
      ...
      // 7. ambient sparks off trail tip
  ...
  }, []); // empty deps — all helpers are module-level pure functions
