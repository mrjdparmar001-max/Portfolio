...
// ── FIX: correct BASE from env, no broken prefix ─────────────────────────────
const BASE = (import.meta.env.VITE_BACKEND_URL || '').replace(/\/+$/, '');

// ── FIX: always returns a displayable URL ─────────────────────────────────────
function imgUrl(raw) {
  if (!raw) return '';
  ...
  return `${BASE}/${s.replace(/^\/+/, '')}`;       // relative → prepend BASE
}
...
function ProjectCard({ project, theme, index }) {
  ...
  // ── FIX: resolve image URL properly ──────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      {/* ── Image ─────────────────────────────────────────────────────────── */}
      <div style={{ position: 'relative', height: 210, overflow: 'hidden', background: theme.bgSecondary }}>
        {src && !imgBroken ? (
          <motion.img
            src={src}
            alt={project.title}
          ...
          <motion.div
            ...
            transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
            style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(135deg, ${theme.primary}25, ${theme.secondary}25)`, fontSize: 52, gap: 8 }}
          >
            💻
            {imgBroken && src && (
              <span style={{ fontSize: 10, color: theme.textMuted, opacity: 0.6, padding: '0 16px', textAlign: 'center', wordBreak: 'break-all' }}>
                {src.slice(0, 50)}
              </span>
            )}
          </motion.div>
        ...
        {project.featured && (
          <motion.div
            ...
            style={{ position: 'absolute', top: 14, right: 14, background: theme.gradient, borderRadius: 20, padding: '4px 14px', fontSize: 11, fontWeight: 700, color: '#fff', backdropFilter: 'blur(8px)', boxShadow: `0 4px 12px ${theme.primary}40` }}
          >
            ⭐ Featured
          </motion.div>
        )}
      ...
      <div style={{ padding: '22px 24px 24px', position: 'relative', zIndex: 3 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          ...
          <motion.span whileHover={{ scale: 1.05 }}
            style={{ background: theme.primary + '22', color: theme.primary, borderRadius: 20, padding: '3px 12px', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap', marginLeft: 8, border: `1px solid ${theme.primary}30` }}>
            {project.category}
          </motion.span>
        </div>
        ...
        <motion.div
          ...
          variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.04, delayChildren: index * 0.07 + 0.2 } } }}
        >
          {project.techStack?.map((tech) => (
            <motion.span key={tech}
              ...
              style={{ background: theme.bgSecondary, color: theme.textMuted, borderRadius: 8, padding: '3px 10px', fontSize: 12, border: `1px solid ${theme.border}`, cursor: 'default', transition: 'background 0.2s, color 0.2s' }}
            >{tech}</motion.span>
          ))}
        </motion.div>
      </div>
...
export default function Projects() {
  ...
  return (
    <section id="projects" style={{ position: 'relative', padding: '100px 5% 90px', background: theme.bg, overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
          ...
          <h2 style={{ fontSize: 'clamp(32px,4vw,52px)', fontWeight: 800, color: theme.text, marginBottom: 6 }}>
            Featured{' '}
            <motion.span initial={{ backgroundSize: '0% 4px' }} whileInView={{ backgroundSize: '100% 4px' }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.6 }}
        ...
        {loading ? (
          ...
          <motion.div layout style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(340px,100%),1fr))', gap: 28 }}>
