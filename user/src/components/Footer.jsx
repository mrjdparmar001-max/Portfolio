    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}>
      ...
      <motion.div
        ...
        style={{
          ...
          width: '50%', height: '100%',
          background: `linear-gradient(90deg, transparent, ${theme.primary}08, ${theme.primary}15, ${theme.primary}08, transparent)`,
        }}
      />
      {/* Grid dots */}
...
// ─── Orbiting particle around social icon ────────────────────────────────────
function OrbitDot({ theme, color, radius = 18, duration = 2 }) {
  return (
    <motion.div
      animate={{ rotate: 360 }}
      transition={{ duration, repeat: Infinity, ease: 'linear' }}
      style={{
        position: 'absolute', inset: -radius,
        pointerEvents: 'none',
      }}
    >
      <div style={{
...
// ─── Social icon with orbit on hover ─────────────────────────────────────────
function SocialIcon({ icon, href, theme, color, delay }) {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {hovered && <OrbitDot theme={theme} color={color} radius={20} duration={1.2} />}
      </AnimatePresence>
    </motion.div>
  );
}
...
function PulsingHeart({ theme }) {
  return (
    <motion.span
      ...
      style={{ display: 'inline-flex', color: theme.secondary }}
    >
...
export default function Footer() {
  ...
  const links = [
    { icon: <FiGithub />,   href: socials.github,            show: !!socials.github,   color: socialColors.github },
    { icon: <FiLinkedin />, href: socials.linkedin,          show: !!socials.linkedin, color: socialColors.linkedin },
    { icon: <FiTwitter />,  href: socials.twitter,           show: !!socials.twitter,  color: socialColors.twitter },
    { icon: <FiMail />,     href: `mailto:${socials.email}`, show: !!socials.email,    color: socialColors.email },
  ].filter(l => l.show);
    ...
    <footer style={{
      ...
      <motion.div
        ...
        style={{
          ...
          width: 300, height: 60,
          background: `radial-gradient(ellipse, ${theme.primary}30, transparent 70%)`,
          filter: 'blur(20px)',
          pointerEvents: 'none',
        }}
      />
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <motion.p
          ...
          style={{
            color: theme.textMuted, fontSize: 14, marginBottom: 4,
        ...
        {links.length > 0 && (
          <div style={{ display: 'flex', gap: 18, justifyContent: 'center', marginBottom: 32 }}>
            {links.map(({ icon, href, color }, i) => (
              <SocialIcon
                key={i} icon={icon} href={href}
                theme={theme} color={color} delay={0.1 * i + 0.4}
              />
            ))}
          </div>
        )}
